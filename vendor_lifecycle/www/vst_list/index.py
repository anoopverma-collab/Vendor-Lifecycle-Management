...
def get_context(context):
	...
	context.title = _("Support Tickets")
	if frappe.session.user == "Guest":
		frappe.throw(_("Login to view your tickets."), frappe.PermissionError)
	...
	tickets = frappe.get_list(
		...
		fields=["name", "subject", "priority", "status", "opened_on"],
		order_by="creation desc",
	)
	for ticket in tickets:
		ticket.opened_on = format_datetime(ticket.opened_on)
