...
class SatisfactionRatingTemplate(Document):
	def validate(self):
		self._validate_no_duplicate_criteria()

	def _validate_no_duplicate_criteria(self):
		# A Criteria listed twice would show up twice on every survey that
		# resolves to this template — confusing, not just redundant.
		seen = set()
