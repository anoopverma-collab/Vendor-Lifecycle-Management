...
LABELS = ['Approved', 'Rejected', 'Draft', 'Vendor Disabled']
DISPLAY_LABELS = ['Approved', 'Rejected', 'Draft', 'Disabled']
...
def get(
	...
	counts = []
	for label in LABELS:
		counts.append(frappe.db.count("Vendor Deboarding Request", filters={"status": label}))

	return {
		"labels": [_(label) for label in DISPLAY_LABELS],
		"datasets": [{"name": "Deboarding Requests by Status", "values": counts}],
	}
