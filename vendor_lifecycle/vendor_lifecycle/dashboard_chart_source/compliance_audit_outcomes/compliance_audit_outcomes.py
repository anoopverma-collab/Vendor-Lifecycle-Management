...
DISPLAY_LABELS = ['Passed', 'Failed', 'Needs Review']
...
def get(
	...
	counts = []
	for label in LABELS:
		counts.append(frappe.db.count("Vendor Compliance Audit", filters={"outcome": label}))

	return {
		"labels": [_(label) for label in DISPLAY_LABELS],
		"datasets": [{"name": "Compliance Audit Outcomes", "values": counts}],
	}
