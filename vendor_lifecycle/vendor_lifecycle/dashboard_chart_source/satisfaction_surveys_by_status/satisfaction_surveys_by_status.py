...
# Fixed label order so each value always gets the same color regardless of
...
# fixed color-per-value mapping impossible to keep stable. The colors
...
LABELS = ['Completed', 'Open', 'In Progress', 'Ratings Given']
DISPLAY_LABELS = ['Completed', 'Open', 'In Progress', 'Ratings Given']
...
def get(
	...
	counts = []
	for label in LABELS:
		counts.append(frappe.db.count("Vendor Satisfaction Survey", filters={"status": label}))

	return {
		"labels": [_(label) for label in DISPLAY_LABELS],
		"datasets": [{"name": "Satisfaction Surveys by Status", "values": counts}],
	}
