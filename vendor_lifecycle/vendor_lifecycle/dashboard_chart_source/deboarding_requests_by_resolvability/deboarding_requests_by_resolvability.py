...
LABELS = ['Yes', 'No', 'Maybe', '']
DISPLAY_LABELS = ['Yes', 'No', 'Maybe', 'Not Set']
...
def get(
	...
	counts = []
	for label in LABELS:
		counts.append(frappe.db.count("Vendor Deboarding Request", filters={"is_resolvable": label}))

	return {
		"labels": [_(label) for label in DISPLAY_LABELS],
		"datasets": [{"name": "Deboarding Requests by Resolvability", "values": counts}],
	}
